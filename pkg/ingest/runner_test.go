...
func TestComputeHoursNoCursor(t *testing.T) {
	...
	hours := computeHours(time.Time{}, 3)
...
func TestComputeHoursCaughtUp(t *testing.T) {
	...
	hours := computeHours(lastAvailable, 5)
...
func TestComputeHoursLookbackCap(t *testing.T) {
	t.Parallel()

	now := time.Now().UTC().Truncate(time.Hour)
	cursor := now.Add(-100 * time.Hour) // way behind

	hours := computeHours(cursor, 5)

	if len(hours) != 5 {
		t.Fatalf("expected 5 hours (capped by lookback), got %d", len(hours))
	}
